...
- `skipUnknownLanguages` (type: `Boolean`, default: `false`) - disable rendering for unknown languages, leaving the code block unchanged.
- `disableWarnings` (type: `Boolean`, default: `false`) - hide warning messages for unknown languages.
- `languageOverrides` (type: `Map<String, Boolean>`, default: `{}`) - explicitly enable/disable rendering of specific languages: `true` to force render, `false` to disable render completely. If no override rules are given, the code block will render if it is a known language or `skipUnknownLanguages` is `false`.
- `defaultLanguage` (type: `String`, default: `txt`) - the language to render a code block in if a language is not specified.
